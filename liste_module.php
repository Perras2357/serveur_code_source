<?php
    ...
    $requet_nom_projet = "select * from projet where id_projet=?";
    $tab = [$_GET["id_projet"]];
    $contenu_nom_projet = requet($requet_nom_projet,$tab);
    
    //session qui contiendra le nom du projet et session id_projet
    $_SESSION["nom_projet"] = $contenu_nom_projet -> nom_projet;
    $_SESSION["id_projet"] = $contenu_nom_projet -> id_projet;


    //requette pour relever tous les modules de ce projet 
    $requet_liste_module = "select * from module where id_projet='".$_GET["id_projet"]."' order by date_create";
    $contenu_liste_module = requet($requet_liste_module);
    ...
    require("view/liste_module.view.php");
